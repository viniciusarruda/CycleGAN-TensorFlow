...
def export_graph(model_name, XtoY=True):
  ...
  with graph.as_default():
    cycle_gan = CycleGAN(ngf=FLAGS.ngf, norm=FLAGS.norm, image_size=(FLAGS.image_height, FLAGS.image_width))

    input_image = tf.placeholder(tf.float32, shape=[FLAGS.image_height, FLAGS.image_width, 3], name='input_image')
    cycle_gan.model()
    if XtoY:
